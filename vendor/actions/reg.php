<?php
    ...
    if(isset($_POST["reg"])){
        ...
        if(!empty($_POST['surname']) && $_POST['name'] && $_POST['patronimyc'] && $_POST['tel'] && $_POST['email'] && $password){
            ...
            if($user -> num_rows > 0){
                ...
                $link -> query ("INSERT INTO `users`(`isAdmin`,`surname`, `name`, `patronimyc`, `tel`, `email`, `password`) 
                VALUES ('0', '{$_POST['surname']}', '{$_POST['name']}', '{$_POST['patronimyc']}', '{$_POST['tel']}', '{$_POST['email']}', '$password')");
